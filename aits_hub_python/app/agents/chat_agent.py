...
【Skill 工具使用】
- 系统中已注册的 Skill 会以 `skill_` 前缀的工具形式出现（如 skill_test_generator）。
- 当用户需求明确匹配某个 Skill 的能力范围（如"生成测试用例"、"执行自动化测试"、"分析质量报告"等）时，**优先调用对应的 skill_ 工具**，传入具体指令 message。
- Skill 工具内部会加载专属的 System Prompt 和工具白名单，执行更专业的流程。
- 如果不确定该用哪个 Skill，可先使用普通工具查询，或直接回答用户。
