...
REVIEW_SYSTEM_PROMPT = """你是一位资深测试评审专家，拥有10年以上测试用例评审经验，熟悉 IEEE 1028 软件评审流程与 ISTQB 测试评审方法。你的任务是对提供的测试用例进行系统性、专业化评审，给出评分、问题列表、遗漏场景和改进建议。
...
## 问题列表
| case_id | case_title | requirement_title | module | issue_type | severity | description | suggestion |
|---------|-----------|-------------------|--------|------------|----------|-------------|------------|
| 15 | 测试完整登录流程 | 用户登录功能需求 | 登录模块 | 完整性 | high | 缺少前置条件描述，未说明用户已注册 | 补充前置条件：用户已注册账号admin/123456 |
| 22 | 异常场景-密码错误 | 用户登录功能需求 | 登录模块 | 可执行性 | medium | 步骤描述不够具体，未说明输入的具体密码 | 修改步骤为：输入错误密码xxx123 |
...
## 冗余性/重复判定规则（最高优先级）
判定两条用例是否构成"重复"，必须同时满足以下三项条件，缺一不可：
1. 需求一致：关联的需求（需求ID）相同
2. 功能点一致：关联的功能点（功能点名称/功能点ID）相同
3. 标题语义相似：用例标题含义相近、指向同一测试点即可（允许措辞、句式、长短差异；只要表达的是同一个测试场景/测试点即视为相似，无需逐字完全一致）
只有以上三者全部满足时，方可判定为重复用例，issue_type 记为"冗余性"。
注意：
- 仅标题相似但需求或功能点不同，不算重复（属于不同需求/功能下的同名用例）；
- 仅步骤相似或预期结果相似但标题语义不同，也不算重复；
- 标题判定看"语义"，不是逐字比对；语义相近即视为相似，例如"测试登录成功流程"与"验证完整登录流程"属相似。
对于判定的重复用例，在 description 中列出所有重复用例的 case_id，并在 suggestion 中建议保留最完整的一条、软删其余重复用例的 case_id。

## 规则
1. score 为 0‑100 的整数，score >= 70 时 passed 为 true，否则为 false；如你显式给出 passed 值则以后者为准。
2. case_id 对应输入用例中标注的数据库ID（即"用例[ID]"中的ID），必须是提供的用例中真实存在的ID。
...
14. 重复判定严格遵循"冗余性/重复判定规则"：需求+功能点一致且标题语义相似才算重复；严禁仅凭标题相同或步骤相似即判定为重复。"""
...
class CaseReviewerAgent(BaseAgent):
    ...
    def _build_human_text(
        ...
        # 3. 待评审用例（按分组组织，使用数据库ID标识）
        parts.append("## 三、待评审用例（使用数据库ID标识，按需求+模块分组）")
        ...
        if groups and len(groups) > 0:
            for g in groups:
                ...
                for case in group_cases:
                    parts.append(self._format_case(case))
        else:
            for case in cases:
                parts.append(self._format_case(case))
    ...
    def _format_case(case: Dict[str, Any]) -> str:
        """格式化单条用例为纯文本，使用数据库ID标识，含需求/功能点用于重复判定"""
        case_id = case.get("id")
        ...
        lines.append(f"  需求ID：{req_id if req_id else '（未关联）'}")
        if feature_name:
            lines.append(f"  功能点：{feature_name}")
        elif feature_module:
            lines.append(f"  功能点模块：{feature_module}")
        else:
            lines.append("  功能点：（未关联）")
        _type_map = {"functional": "功能测试", "performance": "性能测试", "security": "安全测试"}
        lines.append(f"  测试类型：{_type_map.get(case.get('case_type', 'functional'), case.get('case_type', '功能测试'))}")
