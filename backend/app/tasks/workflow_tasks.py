...
- handle_workflow_callback_task: Webhook 回调到达后，按 uuid 定位任务，
  复用 finalize_* 公共函数解析 raw_content 写库
- split_features_workflow_task: 功能点拆分模块的 workflow 后端入口
...
def handle_workflow_callback_task(self, task_id: int, raw_content: str, status: str = "success"):
    ...
    try:
        ...
        logger.exception(f"[workflow_callback] 处理失败: task_id={task_id}, error={e}")
        try:
            task = db.query(AgentTask).filter(AgentTask.id == task_id).first()
...
def split_features_workflow_task(self, task_id: int):
    ...
    try:
        ...
        logger.exception(f"split_features_workflow_task 失败: task_id={task_id}, error={e}")
        try:
            task = db.query(AgentTask).filter(AgentTask.id == task_id).first()
            if task:
                _fallback_to_local(db, task, str(e))
