...
class ApiDocTask(BaseTask):
    """接口文档 AI 生成任务"""

    task_name = "generate_api_doc"
    ...
    def execute(self, db, task_id: int) -> dict:
        ...
        return {
            ...
            "method": (api_def.method or "").upper(),
        ...
        try:
            ...
            notify_event(
                ...
                "ai.api_doc.generated",
                {
                    ...
                    "api_name": result.get("api_name") or "接口",
    ...
    def on_failure(self, db, error: Exception, task_id: int) -> None:
        logger.error(f"AI生成接口文档任务失败: task_id={task_id}, error={error}", exc_info=True)
        try:
            task = db.query(AgentTask).filter(AgentTask.id == task_id).first()
            if task:
